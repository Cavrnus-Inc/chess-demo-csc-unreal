...
UConvertPdfToPdfAsset::UConvertPdfToPdfAsset(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer), WorldContextObject(nullptr), bIsActive(false), PDFFilePath(""), Dpi(0), FirstPage(0), LastPage(0)
{
...
void UConvertPdfToPdfAsset::Activate()
{
	...
	auto ConvertTask = new FAutoDeleteAsyncTask<FAsyncExecTask>([this]() {
		UPDF* PDFAsset = nullptr; // GhostscriptCore->ConvertPdfToPdfAsset(PDFFilePath, Dpi, FirstPage, LastPage);
		if (PDFAsset != nullptr)
		{
